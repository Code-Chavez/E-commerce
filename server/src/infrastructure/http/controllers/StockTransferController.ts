...
const CreateTransferSchema = z.object({
  fromBranchId: z
    .number()
    .int()
    .positive('La sucursal de origen debe ser un ID válido'),
  toBranchId: z
    .number()
    .int()
    .positive('La sucursal de destino debe ser un ID válido'),
  variantId: z
    .number()
    .int()
    .positive('La variante del producto debe ser un ID válido'),
  ...
  async getGuide(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (isNaN(id)) {
        return res
          .status(400)
          .json({ success: false, error: 'ID de transferencia inválido' });
      }
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="guia-transferencia-${id}.pdf"`
      );
