...
import { SubscribeNewsletterSchema, UnsubscribeNewsletterSchema } from "@application/dtos/NewsletterDTOs";
...
export class NewsletterController {
  ...
  async unsubscribe(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const validation = UnsubscribeNewsletterSchema.safeParse({ email: emailInput });
      if (!validation.success) {
        return res.status(400).json({ success: false, error: validation.error.issues });
      }

      const subscriber = await unsubscribeUseCase.execute(validation.data.email);
      return res.status(200).json({ success: true, data: subscriber });
    } catch (error: any) {
