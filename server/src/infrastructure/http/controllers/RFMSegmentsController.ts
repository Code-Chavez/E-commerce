...
import { GetRFMSegmentsUseCase, RFMSegment } from '@application/use-cases/admin/GetRFMSegmentsUseCase';
import { SendRFMCampaignUseCase } from '@application/use-cases/admin/SendRFMCampaignUseCase';

const VALID_SEGMENTS: RFMSegment[] = ['Champions', 'Loyal', 'At Risk', 'Lost', 'Promising'];
...
export class RFMSegmentsController {
  getSegments = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const parsedSegment = segment ? (String(segment) as RFMSegment) : undefined;

      if (parsedSegment && !VALID_SEGMENTS.includes(parsedSegment)) {
        res.status(400).json({ success: false, error: `Segmento inválido. Opciones: ${VALID_SEGMENTS.join(', ')}` });
        return;
      }
  ...
  sendCampaign = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      if (!subject || !body) {
        res.status(400).json({ success: false, error: 'Los campos subject y body son requeridos' });
        return;
      }

      const result = await sendUseCase.execute(segment as RFMSegment, String(subject), String(body));
