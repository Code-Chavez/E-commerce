    ...
    try {
      ...
      if (!['PENDING', 'PROCESSED'].includes(refundStatus)) {
        res.status(400).json({
          success: false,
          error:
            'refundStatus inválido. Valores permitidos: PENDING, PROCESSED',
        });
        return;
      }
      ...
      if (!order) {
        res
          .status(404)
          .json({ success: false, error: `Pedido #${orderId} no encontrado` });
        return;
      }
      ...
      if (order.refundStatus === 'NONE') {
        res.status(409).json({
          success: false,
          error: 'El pedido no tiene un reembolso solicitado',
        });
        return;
      }
