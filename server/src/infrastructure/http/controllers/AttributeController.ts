...
export class AttributeController {
  ...
  async getOne(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!data)
        return res
          .status(404)
          .json({ success: false, error: 'Atributo no encontrado' });
  ...
  async create(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      return res.status(201).json({
        success: true,
        data: await repo.create(parsed.data.name, parsed.data.isVisualDriver),
      });
    } catch (e) {
      next(e);
    }
  ...
  async deactivate(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      return res
        .status(200)
        .json({ success: true, message: 'Atributo inactivado' });
    } catch (e) {
      next(e);
    }
  }
  ...
  async addValue(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      return res.status(201).json({
        success: true,
        data: await repo.addValue(attributeId, parsed.data.value),
      });
    } catch (e) {
      next(e);
    }
  ...
  async deactivateValue(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      return res
        .status(200)
        .json({ success: true, message: 'Valor inactivado' });
    } catch (e) {
      next(e);
    }
  }
