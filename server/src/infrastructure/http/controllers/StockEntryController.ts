...
const StockEntryDistributionItemSchema = z.object({
  branchId: z
    .number()
    .int()
    .positive('El ID de sucursal de destino debe ser un entero positivo'),
  variantId: z
    .number()
    .int()
    .positive('El ID de variante debe ser un entero positivo'),
...
const CreateStockEntrySchema = z.object({
  supplierId: z
    .number()
    .int()
    .positive('El ID de proveedor debe ser un entero positivo'),
  invoiceNumber: z
    .string()
  ...
  distributionItems: z.array(StockEntryDistributionItemSchema).optional(),
});
  ...
  async create(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (error.message?.includes('DISTRIBUTION_EXCEEDS_ENTRY')) {
        return res.status(400).json({
          success: false,
          error: error.message.replace('DISTRIBUTION_EXCEEDS_ENTRY: ', ''),
        });
      }
