...
export class CategoryController {
  ...
  async deactivate(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      return res
        .status(200)
        .json({ success: true, message: 'Categoría inactivada' });
    } catch (e) {
      next(e);
    }
  }
    ...
    try {
      if (!req.file) {
        return res.status(400).json({
          success: false,
          error: 'La imagen de la guía de tallas es requerida',
        });
      }
      ...
      return res.status(500).json({
        success: false,
        error: 'Error al subir la imagen de la guía de tallas a Cloudinary',
      });
    }
  }
}
