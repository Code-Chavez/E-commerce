...
export class SystemSettingCacheService {
  ...
  public static async getSetting(key: string, defaultValue: string): Promise<string> {
    ...
    try {
      ...
      if (setting) {
        this.cache.set(key, { value: setting.value, expiresAt: Date.now() + this.TTL_MS });
        return setting.value;
      }
    } catch (error) {
      console.error(`[SystemSettingCacheService] Error fetching setting ${key}:`, error);
    }

    // Fallback to default
    this.cache.set(key, { value: defaultValue, expiresAt: Date.now() + this.TTL_MS });
    return defaultValue;
  }
