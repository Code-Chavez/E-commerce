...
export class PdfKitShippingLabelService implements IShippingLabelService {
  async generateLabelPdfStream(
    ...
    doc.fontSize(8).fillColor('#6B6B6B').text('CÓDIGO DE ENVÍO', 20, 60);
    ...
    // Recipient Details
    doc.fontSize(8).fillColor('#6B6B6B').text('DESTINATARIO', 20, 155);

    doc.fontSize(11).fillColor('#3F3F3F').text(data.recipientName, 20, 167);
    ...
    // Address Details
    doc.fontSize(8).fillColor('#6B6B6B').text('DIRECCIÓN DE ENTREGA', 20, 205);
    ...
    // District Details
    doc.fontSize(8).fillColor('#6B6B6B').text('DISTRITO / ZONA', 20, 300);
    ...
    doc
      ...
      .fillColor('#6B6B6B')
      .text(
        'Por favor, verifique el contenido del paquete antes de firmar la entrega.',
        20,
        380,
