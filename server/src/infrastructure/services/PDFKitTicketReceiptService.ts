...
export class PDFKitTicketReceiptService {
  generate(receipt: PosReceiptData): PDFKit.PDFDocument {
    ...
    y += 11;
    doc.text(`Sucursal: ${receipt.branch.name}`, M, y, {
      width: CW,
      align: 'center',
    });
    y += 11;
    ...
    doc.fontSize(7).font('Helvetica').text(documentNumber, M, y, { width: CW });
    y += 11;
    doc.text(`Fecha: ${fecha}  ${hora}`, M, y, { width: CW });
    y += 11;
    doc.text(`Doc. Tipo: ${receipt.documentType || 'TICKET'}`, M, y, {
      width: CW,
    });
    y += 11;
    ...
    if (receipt.client) {
      ...
      if (receipt.client.documentId) {
        doc.text(
          `${receipt.client.documentType || 'Doc.'}: ${receipt.client.documentId}`,
          M + 2,
          y,
          { width: CW }
        );
    ...
    for (const item of receipt.items) {
      ...
      doc
        .font('Helvetica')
        .fillColor(gray)
        .text(`x${item.quantity}  @S/ ${item.unitPrice.toFixed(2)}`, M + 4, y, {
          width: CW - 60,
        })
        .text(`S/ ${item.lineTotal.toFixed(2)}`, M, y, {
          width: CW,
          align: 'right',
        });
      y += 11;
      ...
      if (item.discountAmount > 0) {
        doc
          .fillColor('#B91C1C')
          .text(`Desc.: -S/ ${item.discountAmount.toFixed(2)}`, M + 4, y, {
            width: CW,
          });
        y += 11;
      }
    ...
    if (igvExempt) {
      doc
        .text('Op. Inafecta:', M, y, { width: CW - 60 })
        .text(`S/ ${receipt.total.toFixed(2)}`, M, y, {
          width: CW,
          align: 'right',
        });
      y += 13;
      ...
      doc
        .text('Op. Gravada:', M, y, { width: CW - 60 })
        .text(`S/ ${receipt.subtotal.toFixed(2)}`, M, y, {
          width: CW,
          align: 'right',
        });
      y += 13;
      doc
        .text('IGV (18%):', M, y, { width: CW - 60 })
        .text(`S/ ${igvAmount.toFixed(2)}`, M, y, {
          width: CW,
          align: 'right',
        });
      y += 13;
    ...
    if (receipt.discountTotal > 0) {
      doc
        ...
        .text(`-S/ ${receipt.discountTotal.toFixed(2)}`, M, y, {
          width: CW,
          align: 'right',
        });
      y += 13;
    ...
    if (receipt.payments.length > 0) {
      ...
      for (const p of receipt.payments) {
        doc
          ...
          .font('Helvetica')
          .fillColor(gray)
          .text(`${METHOD_LABELS[p.method] || p.method}:`, M, y, {
            width: CW - 70,
          })
          .text(`S/ ${p.amount.toFixed(2)}`, M, y, {
            width: CW,
            align: 'right',
          });
        y += 13;
    ...
    doc.text('¡Gracias por su compra en E-Commerce!', M, y, {
      width: CW,
      align: 'center',
    });
