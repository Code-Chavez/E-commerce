...
export class PDFKitPosReceiptService {
  generate(receipt: PosReceiptData): PDFKit.PDFDocument {
    ...
    const docTypeTitle = receipt.documentType === 'BOLETA' ? 'BOLETA DE VENTA'
                       : receipt.documentType === 'FACTURA' ? 'FACTURA DE VENTA'
                       : 'TICKET DE VENTA';
    ...
    if (receipt.isCrossBranch && receipt.sourceBranch) {
      doc.fillColor('#B91C1C').font('Helvetica-Bold').fontSize(9)
         .text(`PENDIENTE DE RECOJO EN: ${receipt.sourceBranch.name.toUpperCase()}`, 200, 120, { align: 'right', width: 345 });
    ...
    if (igvExempt) {
      doc.text('Op. Inafecta:', 370, y, { width: 110, align: 'right' })
      ...
      doc.text('Op. Gravada:', 370, y, { width: 110, align: 'right' })
         .text(`S/ ${receipt.subtotal.toFixed(2)}`, 490, y, { width: 55, align: 'right' });
      y += 16;
      doc.text('IGV (18%):', 370, y, { width: 110, align: 'right' })
         .text(`S/ ${igvAmount.toFixed(2)}`, 490, y, { width: 55, align: 'right' });
      y += 16;
    }
