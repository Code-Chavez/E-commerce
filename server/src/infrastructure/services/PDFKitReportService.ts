...
export class PDFKitReportService implements IPdfReportService {
  private drawHeader(doc: PDFKit.PDFDocument, title: string) {
    ...
    // Divider line
    doc.moveTo(50, 110).lineTo(545, 110).strokeColor('#D9D9D2').stroke();
  }
  ...
  async generateSalesReport(data: any[]): Promise<NodeJS.ReadableStream> {
    ...
    doc.moveTo(50, 145).lineTo(545, 145).strokeColor('#1A1A2E').stroke();
    ...
    for (const item of data) {
      ...
      doc
        ...
        .text(clientText, 275, currentY, {
          width: 140,
          height: 10,
          ellipsis: true,
        })
        .text(`S/. ${item.total.toFixed(2)}`, 420, currentY, {
          width: 65,
          align: 'right',
        })
    ...
    doc
      .fillColor('#1A1A2E')
      .fontSize(10)
      .text('Total Acumulado:', 300, currentY + 15, {
        width: 120,
        align: 'right',
      })
      .text(`S/. ${totalSalesSum.toFixed(2)}`, 420, currentY + 15, {
        width: 125,
        align: 'right',
      });
  ...
  async generateInventoryReport(data: any[]): Promise<NodeJS.ReadableStream> {
    ...
    for (const item of data) {
      ...
      doc
        ...
        .text(item.productName, 210, currentY, {
          width: 170,
          height: 10,
          ellipsis: true,
        })
        .text(item.branchName, 390, currentY, {
          width: 90,
          height: 10,
          ellipsis: true,
        })
        .text(item.quantity.toString(), 490, currentY, {
          width: 55,
          align: 'right',
        });
  ...
  async generateClientsReport(data: any[]): Promise<NodeJS.ReadableStream> {
    ...
    for (const item of data) {
      ...
      const docText = item.documentType
      ...
      doc
        ...
        .text(fullName, 95, currentY, {
          width: 150,
          height: 10,
          ellipsis: true,
        })
        .text(item.email || '-', 250, currentY, {
          width: 145,
          height: 10,
          ellipsis: true,
        })
