...
import {
  IStockTransferGuideService,
  TransferGuideData,
} from '@domain/services/IStockTransferGuideService';
...
export class PDFKitTransferGuideService implements IStockTransferGuideService {
  generate(data: TransferGuideData): NodeJS.ReadableStream {
    ...
    const hora =
      data.createdAt instanceof Date
        ? data.createdAt.toLocaleTimeString('es-PE', {
            hour: '2-digit',
            minute: '2-digit',
          })
        : new Date(data.createdAt).toLocaleTimeString('es-PE', {
            hour: '2-digit',
            minute: '2-digit',
          });
    ...
    if (data.requestedBy) {
      ...
      doc
        ...
        .fillColor(gray)
        .text(
          `${data.requestedBy.name} ${data.requestedBy.lastName || ''}`.trim(),
          420,
          infoY + 16
        );
    }
    ...
    doc
      .fillColor(dark)
      .fontSize(10)
      .text('Firma Entrega (Origen)', 80, firmY + 10, {
        width: 140,
        align: 'center',
      });
    ...
    doc
      .fillColor(dark)
      .fontSize(10)
      .text('Firma Recepción (Destino)', 375, firmY + 10, {
        width: 140,
        align: 'center',
      });
