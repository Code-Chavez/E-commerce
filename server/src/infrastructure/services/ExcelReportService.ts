...
export class ExcelReportService implements IExcelReportService {
  ...
  async generateNewsletterSubscribersReport(data: any[], format: 'excel' | 'csv'): Promise<NodeJS.ReadableStream> {
    const passThrough = new PassThrough();
    ...
    if (format === 'csv') {
      ...
      for (const item of data) {
        sheet.addRow({
          ...
          subscribedAt: new Date(item.subscribedAt).toISOString().replace('T', ' ').substring(0, 10),
        });
      }
