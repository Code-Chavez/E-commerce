...
export class PDFKitReceiptPdfService implements IReceiptPdfService {
  async generateReceiptPdfStream(
    order: Order,
    user: User
    ...
    doc.moveTo(50, 135).lineTo(545, 135).strokeColor('#D9D9D2').stroke();

    // Sección: Información del Cliente
    doc.fillColor('#3F3F3F').fontSize(11).text('DETALLES DEL CLIENTE', 50, 155);
    ...
    doc.fillColor('#3F3F3F').fontSize(11).text('DIRECCIÓN DE ENVÍO', 300, 155);
    ...
    doc.moveTo(50, 250).lineTo(545, 250).strokeColor('#D9D9D2').stroke();
    ...
    doc.moveTo(50, 275).lineTo(545, 275).strokeColor('#3F3F3F').stroke();
    ...
    for (const item of items) {
      ...
      doc
        ...
        .text(`S/. ${price.toFixed(2)}`, 400, currentY, {
          width: 60,
          align: 'right',
        })
        .text(`S/. ${subtotal.toFixed(2)}`, 475, currentY, {
          width: 70,
          align: 'right',
        });
    ...
    doc
      .text('Envío (Delivery):', 350, currentY + 15, {
        width: 110,
        align: 'right',
      })
      .text(`S/. ${shipping.toFixed(2)}`, 475, currentY + 15, {
        width: 70,
        align: 'right',
      });
    ...
    doc
      ...
      .text(`S/. ${total.toFixed(2)}`, 475, currentY + 35, {
        width: 70,
        align: 'right',
      });
    ...
    doc
      ...
      .text(
        'Este documento es un comprobante digital de compra y no tiene valor tributario.',
        50,
        765,
        { align: 'center' }
      );
