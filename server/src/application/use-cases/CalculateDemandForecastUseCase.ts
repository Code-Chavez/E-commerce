...
export class CalculateDemandForecastUseCase {
  ...
  async execute(months: number): Promise<DemandForecast[]> {
    ...
    const groups: Record<string, { categoryName: string; size: string; totalSalesQty: number }> = {};
    ...
    return Object.values(groups).map(g => ({
      ...
      size: g.size,
      projectedDemand: MovingAverageCalculator.calculate(g.totalSalesQty, months)
    }));
  }
}
