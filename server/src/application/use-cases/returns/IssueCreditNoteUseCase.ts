...
export class IssueCreditNoteUseCase {
  ...
  async execute(returnRequestId: number): Promise<CreditNote> {
    ...
    const existingCreditNote =
      await this.creditNoteRepository.findByReturnRequestId(returnRequestId);
    if (existingCreditNote) {
      throw new Error(
        `A credit note has already been issued for return request ID ${returnRequestId}`
      );
    }
    ...
    if (amount <= 0) {
      throw new Error(
        'Return request has an invalid total amount of 0 or less'
      );
    }
