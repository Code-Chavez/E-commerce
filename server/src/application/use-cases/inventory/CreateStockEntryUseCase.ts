...
export class CreateStockEntryUseCase {
  ...
  async execute(
    ...
    if (!supplier.isActive) {
      throw new Error(
        `El proveedor '${supplier.razonSocial}' se encuentra inactivo`
      );
    }
    ...
    if (dto.distributionItems && dto.distributionItems.length > 0) {
      ...
      for (const item of dto.items) {
        entryQuantities[item.variantId] =
          (entryQuantities[item.variantId] || 0) + item.quantity;
      }

      // Sumar distribución y comparar
      const distQuantities: Record<number, number> = {};
      for (const dist of dto.distributionItems) {
        distQuantities[dist.variantId] =
          (distQuantities[dist.variantId] || 0) + dist.quantity;
      }
