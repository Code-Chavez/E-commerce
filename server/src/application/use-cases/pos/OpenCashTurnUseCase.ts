...
import {
  OpenCashTurnRequestDTO,
  CashTurnResponseDTO,
} from '../../dtos/CashTurnDTOs';
...
export class OpenCashTurnUseCase {
  ...
  async execute(dto: OpenCashTurnRequestDTO): Promise<CashTurnResponseDTO> {
    ...
    if (!register) {
      throw new Error(
        `La caja registradora con ID ${dto.registerId} no existe`
      );
    }

    // 2. Validar que la caja no esté ocupada por un turno activo
    const activeRegisterTurn =
      await this.cashTurnRepository.findActiveByRegister(dto.registerId);
    if (activeRegisterTurn) {
      throw new Error(
        `La caja registradora '${register.name}' ya tiene un turno abierto activo`
      );
    }

    // 3. Validar que el usuario no tenga ya un turno abierto
    const activeUserTurn = await this.cashTurnRepository.findActiveByUser(
      dto.userId
    );
    if (activeUserTurn) {
      throw new Error(
        `El usuario ya tiene un turno abierto en la caja con ID ${activeUserTurn.registerId}`
      );
    }
