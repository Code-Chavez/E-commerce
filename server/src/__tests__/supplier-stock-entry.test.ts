...
jest.mock('@application/use-cases/supplier/GetAllSuppliersUseCase', () => ({
  GetAllSuppliersUseCase: jest.fn().mockImplementation(() => ({
    execute: jest
      .fn()
      .mockResolvedValue([
        { id: 1, razonSocial: 'Importadora Sur', ruc: '20123456789' },
      ] as never),
  })),
}));
...
jest.mock('@application/use-cases/inventory/CreateStockEntryUseCase', () => {
  const { CreateStockEntryUseCase } = jest.requireActual<any>(
    '@application/use-cases/inventory/CreateStockEntryUseCase'
  );
  return {
...
describe('Supplier and Stock Entry API (HU-051)', () => {
  ...
  describe('Suppliers', () => {
    ...
    it('should create supplier successfully (Happy Path)', async () => {
      ...
      const response = await request(app).post('/api/v1/suppliers').send({
        razonSocial: 'Distribuidora Central',
        ruc: '20987654321',
        contacto: 'Juan',
        rubro: 'Lácteos',
  ...
  describe('Stock Entries', () => {
    it('should register a stock entry successfully (Happy Path)', async () => {
      ...
      (prisma.stockEntry.create as any).mockResolvedValue({
        ...
        createdAt: new Date(),
        updatedAt: new Date(),
        items: [{ id: 10, variantId: 1, quantity: 100, unitCost: 15.5 }],
        supplier: {
          id: 1,
          razonSocial: 'Supplier',
          ruc: '20123456789',
      ...
      const payload = {
        ...
        branchId: 1,
        invoiceNumber: 'F001-00001234',
        items: [{ variantId: 1, quantity: 100, unitCost: 15.5 }],
      };
      const response = await request(app)
    ...
    it('debe rechazar con HTTP 400 si la distribución excede el ingreso (HU-022)', async () => {
      ...
      expect(response.body.error).toContain(
        'La cantidad a distribuir (110) excede el total ingresado (100) para la variante 1'
      );
    });
  });
  ...
  describe('PrismaStockEntryRepository (HU-026 / HU-051)', () => {
    it('Debe hacer rollback del StockEntry si falla la creación del KardexEntry', async () => {
      const { PrismaStockEntryRepository } =
        await import('@infrastructure/database/repositories/PrismaStockEntryRepository');
      ...
      (prisma.kardexEntry.create as any).mockRejectedValue(
        new Error('KardexEntry DB Error')
      );
