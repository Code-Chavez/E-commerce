...
describe('DeliveryStateMachine', () => {
  ...
  it('should allow the failed-delivery decision flow transitions', () => {
    expect(() => DeliveryStateMachine.validateTransition('FAILED', 'AWAITING_CLIENT_DECISION')).not.toThrow();
    expect(() => DeliveryStateMachine.validateTransition('AWAITING_CLIENT_DECISION', 'RETURNED')).not.toThrow();
    expect(() => DeliveryStateMachine.validateTransition('AWAITING_CLIENT_DECISION', 'PENDING')).not.toThrow();
  });

  it('should reject invalid transitions from AWAITING_CLIENT_DECISION', () => {
    expect(() => DeliveryStateMachine.validateTransition('AWAITING_CLIENT_DECISION', 'DELIVERED'))
      .toThrow(InvalidDeliveryStateTransitionError);
    expect(() => DeliveryStateMachine.validateTransition('AWAITING_CLIENT_DECISION', 'IN_TRANSIT'))
      .toThrow(InvalidDeliveryStateTransitionError);
  });
