  ...
  return {
    ...
    default: {
      $transaction: jest.fn(async (cb: any) => cb({
        ...
        posOrder: {
          create: jest.fn().mockResolvedValue({ id: 1, branchId: 1, total: 100, cashTurnId: 1 } as never),
        },
        payment: {
          createMany: jest.fn(),
        },
...
jest.mock('@infrastructure/http/middlewares/auth.middleware', () => ({
  requireAuth: (req: any, res: any, next: any) => { req.auth = { userId: 1, role: 'SELLER', branchId: 1 }; next(); },
  requirePermission: () => (req: any, res: any, next: any) => { req.auth = { userId: 1, role: 'SELLER', branchId: 1 }; next(); },
  optionalAuth: (req: any, res: any, next: any) => { req.auth = { userId: 1, role: 'SELLER', branchId: 1 }; next(); }
}));
...
describe('Sale POS - Cash Turn Validation (HU-032)', () => {
  ...
  it('debe rechazar con HTTP 403 si el vendedor no tiene turno de caja abierto', async () => {
    ...
    (prisma.cashTurn.findFirst as jest.Mock).mockResolvedValueOnce(null as never);
    ...
    expect(response.body.error).toContain('No tienes un turno de caja abierto para registrar ventas');
  });

  it('debe registrar la venta con HTTP 201 y asociarla al turno si existe turno abierto', async () => {
    // Simular que hay turno abierto
    (prisma.cashTurn.findFirst as jest.Mock).mockResolvedValueOnce({ id: 99, status: 'OPEN', userId: 1 } as never);
